...
(async function () {
  try {
    ...
    await exec(
      `C:/Users/pc/Downloads/user/bin/ffmpeg -i public/videos/test.mp4 temp/raw-frames/%d.png`
    );
    ...
    for (let count = 1; count <= frames.length; count++) {
      ...
      //   `temp/raw-frames/${count}.png`,
      //   function (err, image) {
      //     if (err) throw err;
      //     image.resize(256, 256).quality(50);
    ...
    await exec(
      `C:/Users/pc/Downloads/user/bin/ffmpeg -start_number 1 -i temp/edited-frames/%d.png -vcodec ${videoEncoder} -filter:v "setpts=0.5*PTS" temp/no-audio.mp4`
    );

    console.log("Adding audio");
    await exec(
      `C:/Users/pc/Downloads/user/bin/ffmpeg -i temp/no-audio.mp4 -i public/videos/test.mp4 -c copy -map 0:v:0 -map 1:a:0 ${output}`
    );
